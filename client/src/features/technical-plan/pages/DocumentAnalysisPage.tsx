...
function DocumentAnalysisPage({
  ...
  const { confirmAsync: confirmWipeAction, dialogElement: wipeConfirmDialog } = useConfirmAction();
  ...
  const importTenderDocument = async () => {
    if (hasGeneratedOutline) {
      const ok = await confirmWipeAction({
        title: '替换招标文件将清空现有进度',
        description: '替换招标文件会清除已生成的目录与全部正文，并重置到招标解析步骤。此操作不可撤销。是否继续？',
        confirmText: '确认替换',
      });
      if (!ok) {
        return;
  ...
  const importOriginalPlanDocument = async () => {
    if (hasGeneratedOutline) {
      const ok = await confirmWipeAction({
        title: '重新导入原始方案将清空现有进度',
        description: '重新导入原始方案会清除已生成的目录与全部正文，并重置到招标解析步骤。此操作不可撤销。是否继续？',
        confirmText: '确认重新导入',
      });
      if (!ok) {
        return;
