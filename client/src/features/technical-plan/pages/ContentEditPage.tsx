...
function ContentEditPage({
  ...
  const strictSectionWords = contentStats?.strict_section_words === true;
  const wordControlTargetLabel = minimumWords > 0 && maximumWords > 0
  ...
  const shouldAskMinimumWordsChoice = () => leaves.length > 0
    && completedCount === leaves.length
    && !canRetryMinimumWords
    && minimumWords > 0
  ...
  const renderTree = (items: OutlineItem[], level = 0): ReactNode => items.map((item) => {
    ...
    return (
      <div
        className="content-outline-node"
        key={item.id}
        style={{ '--content-level': Math.min(level, 4) } as CSSProperties}
      >
        <button
          type="button"
    ...
    <div className={`plan-step-body content-generation-page${showIllustrationStats ? ' has-dev-stats' : ''}`}>
      ...
      <section className="content-generation-command-bar">
        ...
        <div className="content-generation-stats" aria-label="正文生成统计">
          ...
          {(minimumWords > 0 || maximumWords > 0 || sectionWords > 0) && (
            <span>
              <strong>{wordControlTargetLabel}</strong>
              字数目标{sectionWords ? ` · 小节${sectionWords}${strictSectionWords ? '强控' : ''}` : ''}
            </span>
          )}
        </div>
          ...
          <Dialog.Content className="content-generation-config-card" aria-describedby={undefined}>
            ...
            <div className="content-generation-config-list">
              ...
              <div className="content-generation-config-note">
                全文字数、最多字数和每小节字数由 STEP 03「目录生成配置」统一控制；如需调整，请回到 STEP 03 修改并重新生成目录后再生成正文。
              </div>
              <label className="content-generation-config-row">
                <span>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="content-generation-config-card">
            <div className="content-regenerate-card-head">
              ...
              <Dialog.Description>
                当前约 {pendingMinimumWordsChoice?.currentWords ?? totalWords} 字，STEP 03 设置的最低字数为 {pendingMinimumWordsChoice?.minimumWords ?? 0} 字。可以保留现有正文继续补齐，也可以清空后重新生成。
              </Dialog.Description>
            </div>
            <div className="content-generation-config-note">
