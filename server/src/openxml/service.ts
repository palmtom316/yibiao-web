...
export async function runOpenXmlJob(workspace: string, request: Record<string, unknown>, timeoutMs = 30000, signal?: AbortSignal) {
  ...
  const output = await new Promise<string>((resolve, reject) => {
    ...
    const onAbort = () => { child.kill('SIGKILL'); reject(new ApiError(409, '任务已取消，原件和成功版本保留')); };
    ...
    child.once('error', () => { clearTimeout(timer); signal?.removeEventListener('abort', onAbort); reject(new ApiError(503, '模板抽取工具未安装或不可启动')); });
    child.once('exit', (code) => { clearTimeout(timer); signal?.removeEventListener('abort', onAbort); if (code !== 0) reject(new ApiError(422, '模板抽取工具运行失败')); else resolve(stdout); });
  ...
  if (!done?.ok || !result.ok) throw Object.assign(new ApiError(422, '未能从该 Word 抽取模板，仍可生成普通目录'), { internalDetails: result });
...
export async function extractTemplate(prisma: PrismaClient, projectId: number, userId: number, sourceId: string, selectedTitles?: string[], signal?: AbortSignal) {
  if (signal?.aborted) throw new ApiError(409, '任务已取消，原件和成功版本保留');
  ...
  return parseQueue.run(async () => {
    ...
    try {
      ...
      const listed = await runOpenXmlJob(workspace, { action: 'list-blocks', sources }, 30000, signal);
      ...
      await runOpenXmlJob(workspace, { action: 'extract-chapters', sources, chapters, output: 'output/template.docx' }, 30000, signal);
      const scanned = await runOpenXmlJob(workspace, { action: 'scan-template-fields', input: 'output/template.docx' }, 30000, signal);
      ...
      if (signal?.aborted || (error instanceof ApiError && error.statusCode === 409 && /取消/.test(error.message))) throw error;
