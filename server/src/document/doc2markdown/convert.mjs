...
const PDF_TABLE_HEADER_TERM_PATTERN = /(?:序号|编号|名称|等级|类别|类型|项目|内容|要求|指标|参数|规格|数量|单位|备注|说明|条目|分值|得分)/;

function parsePdfTableSerial(value) {
  const normalized = collapsePdfWhitespace(String(value || '').replace(/<br\s*\/?\s*>/gi, ''));
  const match = /^(\d+)(?:[.．、)）])?$/.exec(normalized);
  return match ? Number(match[1]) : null;
}

function isLikelyPdfTableHeaderRow(row) {
  const cells = (row || []).map((cell) => collapsePdfWhitespace(String(cell || '').replace(/<br\s*\/?\s*>/gi, '')));
  if (cells.length < 2 || parsePdfTableSerial(cells[0]) !== null) {
    return false;
  }
  const matchedCells = cells.filter((cell) => PDF_TABLE_HEADER_TERM_PATTERN.test(cell)).length;
  return /^(?:序号|编号)$/.test(cells[0]) || matchedCells >= 2;
}

function pdfTableWidth(table) {
  return Math.max(0, ...(table || []).map((row) => Array.isArray(row) ? row.length : 0));
}

function lastPdfTableSerial(table) {
  for (let index = (table || []).length - 1; index >= 1; index -= 1) {
    const serial = parsePdfTableSerial(table[index]?.[0]);
    if (serial !== null) return serial;
  }
  return null;
}

function shouldStitchCrossPagePdfTables(previousTable, nextTable) {
  const previousWidth = pdfTableWidth(previousTable);
  const nextWidth = pdfTableWidth(nextTable);
  if (previousWidth < 2 || previousWidth !== nextWidth || !isLikelyPdfTableHeaderRow(previousTable?.[0])) {
    return false;
  }
  if (isLikelyPdfTableHeaderRow(nextTable?.[0])) {
    return false;
  }
  const nextSerial = parsePdfTableSerial(nextTable?.[0]?.[0]);
  if (nextSerial === null) {
    return false;
  }
  const previousSerial = lastPdfTableSerial(previousTable);
  return previousSerial === null ? nextSerial === 1 : nextSerial === previousSerial + 1;
}

export function stitchCrossPagePdfTables(pages) {
  const result = (pages || []).map((page) => (page || []).map((table) =>
    (table || []).map((row) => Array.isArray(row) ? [...row] : [])));
  for (let pageIndex = 0; pageIndex + 1 < result.length; pageIndex += 1) {
    const previousPage = result[pageIndex];
    const nextPage = result[pageIndex + 1];
    if (!previousPage.length || !nextPage.length) continue;
    const previousTableIndex = previousPage.length - 1;
    const previousTable = previousPage[previousTableIndex];
    const nextTable = nextPage[0];
    if (!shouldStitchCrossPagePdfTables(previousTable, nextTable)) continue;
    previousPage[previousTableIndex] = [...previousTable, ...nextTable];
    nextPage.shift();
  }
  return result;
}

function collectUniquePdfTables(tables) {
  const rows = [];
  const rendered = [];
  for (const table of tables || []) {
    const normalizedRows = normalizePdfTableRows(table);
    const markdown = renderMarkdownTable(normalizedRows);
    if (markdown && !hasSimilarPdfTable(rendered, markdown)) {
...
async function renderPdfMarkdown(textResult, tableResult, pdfJsTableResult, imageResult, includeImages, imageResolver) {
  ...
  const parts = [];
  const sourceTablesByPage = Array.from({ length: pageCount }, (_, index) => collectUniquePdfTables([
    ...(tablePages[index]?.tables || []),
    ...(pdfJsTablePages[index]?.tables || []),
  ]));
  const renderedTablesByPage = stitchCrossPagePdfTables(sourceTablesByPage);

  for (let index = 0; index < pageCount; index += 1) {
    const pageParts = [];
    const pageText = normalizePdfPlainText(textPages[index]?.text || '');
    const sourceTableMarkdownList = sourceTablesByPage[index].map((table) => renderMarkdownTable(table)).filter(Boolean);
    const renderedTableMarkdownList = renderedTablesByPage[index].map((table) => renderMarkdownTable(table)).filter(Boolean);

    const dedupedText = removePdfTableDuplicateText(pageText, sourceTableMarkdownList);
    if (dedupedText) {
      pageParts.push(dedupedText);
    }
    pageParts.push(...renderedTableMarkdownList);
