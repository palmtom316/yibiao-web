...
export async function parseFileInBoundedWorkerUnqueued(
  sourcePath: string,
  outputDir: string,
  signal?: AbortSignal,
...
export async function parseFileInBoundedWorker(
  ...
  return parseQueue.run(() => parseFileInBoundedWorkerUnqueued(sourcePath, outputDir, signal), undefined, signal);
}
