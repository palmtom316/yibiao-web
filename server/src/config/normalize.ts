// 从 client/electron/services/configStore.cjs 原样移植的纯配置逻辑（无 I/O）。
// 持久化由 store.ts 改走 Prisma；归一化/默认值/校验逻辑保持等价，避免行为漂移。
...
const DEFAULT_TEXT_REQUEST_QUEUE_LIMIT = (() => {
...
const defaultTextModelProfiles: any = {
  jinlong: {
    api_key: '',
...
const legacyTextModelProfiles: any = {
  longcat: {
    api_key: '',
...
const defaultImageModelProfiles: any = {
  jinlong: {
    provider: 'jinlong',
...
function createAnalyticsClientId(): string {
  return randomUUID();
}

function createAnalyticsCreatedAt(): string {
  ...
  const values: any = Object.fromEntries(parts.map((part) => [part.type, part.value]));
  return `${values.year}-${values.month}-${values.day}`;
}

function isTextModelProvider(value: any): boolean {
  return textModelProviders.includes(value);
}

function isLegacyTextModelProvider(value: any): boolean {
  return legacyTextModelProviders.includes(value);
}

function isImageModelProvider(value: any): boolean {
  return imageModelProviders.includes(value);
}

function normalizeAiRequestMode(value: any, fallback = 'stream'): string {
  return aiRequestModes.includes(value) ? value : fallback;
}

function normalizeUpdateChannel(value: any, fallback: string = defaultConfig.update_channel): string {
  return updateChannels.includes(value) ? value : fallback;
}

function normalizeTextContextLengthLimit(value: any, fallback = DEFAULT_TEXT_CONTEXT_LENGTH_LIMIT): number {
  const number = Number(value);
  return Number.isFinite(number) && number > 0 ? Math.floor(number) : fallback;
}

function normalizeTextConcurrencyLimit(value: any, fallback = DEFAULT_TEXT_CONCURRENCY_LIMIT): number {
  const number = Number(value);
  return Number.isFinite(number) && number > 0 ? Math.round(number) : fallback;
}

function normalizeTextRequestQueueLimit(value: any, fallback = DEFAULT_TEXT_REQUEST_QUEUE_LIMIT): number {
  const number = Number(value);
  return Number.isFinite(number) && number > 0 ? Math.round(number) : fallback;
}

function normalizeImageConcurrencyLimit(value: any, fallback = DEFAULT_IMAGE_CONCURRENCY_LIMIT): number {
  const number = Number(value);
  return Number.isFinite(number) && number > 0 ? Math.round(number) : fallback;
}

function normalizeTextModelProfile(provider: any, profile: any): any {
...
function normalizeTextModelProfiles(sourceProfiles: any): any {
  const profiles: any = {};
...
function textProfileFromFlatConfig(source: any, fallback: any, provider: any): any {
  const sourceBaseUrl = provider === 'custom'
...
function textProfileFromUnknownProvider(source: any, sourceProvider: any, fallback: any): any {
  ...
  return {
    ...
    concurrency_limit: normalizeTextConcurrencyLimit(pickTextProfileField(source.concurrency_limit, selectedProfile?.context_length_limit, fallback.concurrency_limit), fallback.concurrency_limit),
...
function getImageSizeOptions(provider: any): string[] {
  return provider === 'google-ai-studio' ? googleImageSizes : openAICompatibleImageSizes;
}

function normalizeImageSize(provider: any, value: any, fallback: any): string {
...
function normalizeImageModelProfile(provider: any, profile: any): any {
...
function normalizeImageModelProfiles(sourceProfiles: any): any {
  const profiles: any = {};
...
function normalizeAgentModeScenarios(source: any): any {
...
function cloneDefaultExportFormat(def: any = defaultExportFormat): any {
  return {
    ...
    headings: def.headings.map((heading: any) => ({ ...heading })),
...
function normalizeTableCellStyle(source: any, def: any): any {
...
function normalizeImageStyle(source: any, def: any): any {
  const src = source && typeof source === 'object' ? source : {};
  return {
...
function normalizeExportFormat(source: any): any {
  ...
  const page = {
    ...
    header_color: typeof srcPage.header_color === 'string' ? srcPage.header_color : def.page.header_color,
    ...
    footer_color: typeof srcPage.footer_color === 'string' ? srcPage.footer_color : def.page.footer_color,
  ...
  const heading_border = {
    ...
    level_cell_colors: defHeadingCellColors.map((color: string, index: number) => (typeof srcHeadingCellColors[index] === 'string' && srcHeadingCellColors[index] ? srcHeadingCellColors[index] : color)),
  ...
  const headings = def.headings.map((defH: any, i: number) => {
...
export function normalizeConfig(config?: any): any {
  ...
  return {
    ...
    text_request_queue_limit: normalizeTextRequestQueueLimit(source.text_request_queue_limit),
    ...
    system_name: typeof source.system_name === 'string' && source.system_name.trim() ? source.system_name.trim() : defaultConfig.system_name,
    logo_data_url: typeof source.logo_data_url === 'string' && source.logo_data_url ? source.logo_data_url : defaultConfig.logo_data_url,
  };
}

export function withAnalyticsIdentity(config: any): any {
...
export { defaultConfig, defaultExportFormat };
