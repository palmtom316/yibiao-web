...
// Web adapter replaces Electron rendering; generation and repair algorithms are retained.
const {
...
} = require('./webImageRenderAdapter.cjs');
...
function buildGeneratedIllustrationMarkdown(planItem) {
  ...
  let body = '';
  // Web always prefers a persisted, authorized project image. Code remains for retries.
  if (planItem.kind === 'mermaid' && generation.code && !generation.asset_url) {
